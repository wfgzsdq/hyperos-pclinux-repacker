...
def assets(cfg,image,dest):
 ...
 digest=hashlib.file_digest(image.open('rb'),'sha256').hexdigest();c['image_sha256']=digest;c['image_bytes']=image.stat().st_size
 c['image_path']='/data/rootfs/pclimg/'+digest[:16]+'.erofs'
 dest.mkdir(parents=True,exist_ok=True)
 target_image=dest/'application.erofs'
 if not target_image.exists() or hashlib.file_digest(target_image.open('rb'),'sha256').hexdigest()!=digest:shutil.copyfile(image,target_image)
 ...
 args=[x.replace('@APP@',app).replace('@DATA@',data) for x in c['args']]
