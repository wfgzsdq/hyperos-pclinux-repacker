...
"""
import argparse,gzip,hashlib,io,json,posixpath,re,shlex,struct,sys,tarfile,uuid,time
...
def prepare(deb,config,out):
 digest=hashlib.file_digest(deb.open('rb'),'sha256').hexdigest()
 ...
 with tarfile.open(fileobj=io.BytesIO(payload)) as src, (out/'payload.tar.gz').open('wb') as raw, gzip.GzipFile(fileobj=raw,mode='wb',compresslevel=1,mtime=0) as gz, tarfile.open(fileobj=gz,mode='w|',format=tarfile.PAX_FORMAT) as dest:
  for m in src:
...
def build_image(payload,out,config):
 ...
 adb.shell('su -c '+shlex.quote('mkdir -p '+remote+'; chown 2000:2000 '+remote))
 ...
 imageuuid=str(uuid.uuid5(uuid.NAMESPACE_URL,config['deb_sha256']))
...
'''
 ...
 result=adb.shell('su -c '+shlex.quote('sh '+remote+'/build-image.sh; rc=$?; echo __RESULT=$rc'),timeout=600)
